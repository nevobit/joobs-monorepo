...
export const blockUser = async (data: InferInsertModel<typeof blocks>) => {
    ...
    const existingConnection = await db.select().from(blocks).where(or(and(eq(blocks.senderId, data.senderId), eq(blocks.receiverId, data.receiverId)), and(eq(blocks.senderId, data.receiverId), eq(blocks.receiverId, data.senderId))));
